...
import data.techPanda.AccountData;
...
public class Level_25_Environment_XML extends BaseTest {

    @Parameters({"browser","server","role"})
    @BeforeClass
    public void beforeClass(String browserName,String serverName,String role) {
        driver = getBrowserDriver(browserName,serverName,role);
        showBrowserConsoleLogs();
        homePage = PageGeneratorManager.getUserHomePage(driver);

        accountData = AccountData.getAccountData();
        emailAdress = accountData.getEmailAddress() + getRandomNumber() + accountData.getWebEmailServer();
        password = "123456";
    ...
    String emailAdress, password, firstName, lastName, fullName;
    DataFaker faker;
